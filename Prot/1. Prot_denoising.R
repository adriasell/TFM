###############################################################################
##################### Proteins preprocessing & denoising ######################
###############################################################################
...
pData(omics$data) <- dummy_cols(pData(omics$data),
                                select_columns = "Prot.plate", 
                                remove_selected_columns = F)

prot_dummies <- names(pData(omics$data))[grep("Prot.plate_", names(pData(omics$data)))]
prot_dummies <- prot_dummies[-length(prot_dummies)]
pData(omics$data)[prot_dummies] <- lapply(pData(omics$data)[prot_dummies], as.factor)
...
denoise_vars_n <- c(prot_dummies, "e3_bw", "hs2_visit_age_years_Time1",
                    "hs_dift_mealblood_imp", "e3_sex", "h_ethnicity_c_Asian_pakistani", 
                    "h_ethnicity_c_Caucasian",'cohort_BIB', "cohort_EDEN", "cohort_KANC")

denoise_vars_s <- c(prot_dummies, "e3_bw", "hs2_visit_age_years_Time1",
                    "hs_dift_mealblood_imp", "e3_sex", "cohort_SAB")

denoise_vars <- c(prot_dummies, "e3_bw", "hs2_visit_age_years_Time1","hs_dift_mealblood_imp",
                  "e3_sex","h_ethnicity_c_Asian_pakistani","h_ethnicity_c_Caucasian",'cohort_BIB', 
...
# ///// Save denoised data
#saveRDS(omics_n_denoised, file = paste0(output_path, "/prot_denoised_n.RDS"))
#saveRDS(omics_s_denoised, file = paste0(output_path, "/prot_denoised_s.RDS"))
saveRDS(omics_denoised, file = paste0(output_path, "/prot_denoised.RDS"))
...
omics_n_denoised <- readRDS(paste0(output_path, "/prot_denoised_n.RDS"))
omics_s_denoised <- readRDS(paste0(output_path, "/prot_denoised_s.RDS"))
omics_denoised <- readRDS(paste0(output_path, "/prot_denoised.RDS"))
