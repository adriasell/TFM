...
library(Deriv)
library(minfi)
#install.packages("./packages_R/RGCCA-main", repos = NULL, type = "source") Install RGCCA modified so sparsity can be 0
library(RGCCA)
...
### Methylation
methyl_all <- eset2df("./results/denoising/denoising_methyl/methyl_denoised.RDS")
methyl_all$HelixID <- rownames(methyl_all)
methyl_n <- methyl_all[rownames(methyl_all) %in% helixid_n,]
methyl_s <- methyl_all[rownames(methyl_all) %in% helixid_s,]
...
ids_s <- Reduce(intersect, list(prot_s$HelixID, serum_s$HelixID, urine_s$HelixID, methyl_s$HelixID, Y$HelixID))
...
methyl_n <- methyl_n %>% arrange(HelixID) %>% dplyr::select(-HelixID)
methyl_s <- methyl_s %>% arrange(HelixID) %>% dplyr::select(-HelixID)
methyl_all <- methyl_all %>% arrange(HelixID) %>% dplyr::select(-HelixID)
Y <- Y %>% 
...
X_north <- list(prot = prot_n[rownames(prot_n) %in% ids_n,],
                ...
                methyl= methyl_n[rownames(methyl_n) %in% ids_n,],
...
X_south <- list(prot = prot_s[rownames(prot_s) %in% ids_s,],
                ...
                methyl= methyl_s[rownames(methyl_s) %in% ids_s,],
...
X_combined <- list(prot = prot_all[rownames(prot_all) %in% c(ids_s, ids_n),],
                   ...
                   methyl= methyl_all[rownames(methyl_all) %in% c(ids_s, ids_n),],
...
write_rds(list_X, file = "./results/RGCCA_methyl/list_X.rds")
...
bootstrap <- readRDS("./results/RGCCA_methyl/model/bootstrap.rds")
...
png("./results/RGCCA_methyl/LC_bootstrap/bootstrap_prot.png")
bootstrap_figure(bootstrap$stats, block = "prot", color = "#E05600", type = "weights")
dev.off()

png("./results/RGCCA_methyl/LC_bootstrap/bootstrap_serum.png")
bootstrap_figure(bootstrap$stats, block = "serum", color = "#10450F")
dev.off()

png("./results/RGCCA_methyl/LC_bootstrap/bootstrap_urine.png")
bootstrap_figure(bootstrap$stats, block = "urine", color = "#064550" )
dev.off()

png("./results/RGCCA_methyl/LC_bootstrap/bootstrap_methyl.png")
bootstrap_figure(bootstrap$stats, block = "methyl" )
dev.off()

png("./results/RGCCA_methyl/LC_bootstrap/bootstrap_Y.png")
...
write_rds(results_list_validation, file = "./results/RGCCA_methyl/model_results/results_rgcca_validation.rds")
...
#R2 block
png("./results/RGCCA_methyl/R2/R2_block_all.png", width = 1500, height = 588)
...
png("./results/RGCCA_methyl/R2/R2_block_n.png", width = 1500, height = 588)
...
png("./results/RGCCA_methyl/R2/R2_block_s.png", width = 1500, height = 588)
...
#R2 
png("./results/RGCCA_methyl/R2/R2_outcome_all.png", width = 1500, height = 588)
...
png("./results/RGCCA_methyl/R2/R2_outcome_n.png", width = 1500, height = 588)
...
png("./results/RGCCA_methyl/R2/R2_outcome_s.png", width = 1500, height = 588)
