...
imp_value<-mean(omics$data$hs_dift_mealblood_imp, na.rm = T)
...
array_dummies <- names(pData(omics$data))[grep("Methyl.Array_", names(pData(omics$data)))]
array_dummies <- array_dummies[-length(array_dummies)]
pData(omics$data) <- DataFrame(dummy_cols(pData(omics$data), 
                                          select_columns = "Methyl.BATCH_Conversion",
                                          remove_selected_columns = FALSE), row.names = pData(omics$data)$SampleID)

batch_dummies <- names(pData(omics$data))[grep("Methyl.BATCH_Conversion_", names(pData(omics$data)))]
...
denoise_vars_n <- c(array_dummies, batch_dummies, "CD8T", "CD4T", "NK", "Bcell", "Mono", "Neu", "NK", "Eos",
                    "e3_bw", "hs2_visit_age_years_Time1", "e3_sex","h_ethnicity_c_Asian_pakistani", 
                    "h_ethnicity_c_Caucasian", 'cohort_BIB', "cohort_EDEN", "cohort_KANC")

denoise_vars_s <- c(array_dummies, batch_dummies, "CD8T", "CD4T" , "NK", "Bcell", "Mono","NK", "Eos", 
                    "Neu", "e3_bw", "hs2_visit_age_years_Time1", "e3_sex", "cohort_SAB")

denoise_vars <- c(array_dummies, batch_dummies, "CD8T", "CD4T", "NK", "Bcell", "Mono", "Neu", "NK", "Eos",
                  "e3_bw", "hs2_visit_age_years_Time1", "e3_sex","h_ethnicity_c_Asian_pakistani",
...
methyl_data<-list("omics_n"=omics_n,
                  "denoise_vars_n"=denoise_vars_n,
                  "omics_s"=omics_s,
                  "denoise_vars_s"=denoise_vars_s,
                  "omics"=omics,
                  "denoise_vars"=denoise_vars)

save(methyl_data, file = "./results/denoising/denoising_methyl/methyl_data.RData")
#load("./results/denoising/denoising_methyl/methyl_data.RData")

omics_n_denoised <- denoising_mlmer(omics_n, omicsLayer, denoise_vars_n, ncores = detectCores(), blocksize = 100)
omics_s_denoised <- denoising_mlmer(omics_s, omicsLayer, denoise_vars_s, ncores = detectCores(), blocksize = 100)
omics_denoised <- denoising_mlmer(omics$data, omicsLayer, denoise_vars, ncores = detectCores(), blocksize = 100)

# ///// Save denoised data
saveRDS(omics_n_denoised, file = paste0(output_path, "/methyl_denoised_n.RDS"))
saveRDS(omics_s_denoised, file = paste0(output_path, "/methyl_denoised_s.RDS"))
