...
# ···· Modify with dummies groups and factors
pData(omics$data) <- dummy_cols(pData(omics$data),
                                select_columns = "Serum.run_order1", 
                                remove_selected_columns = F)

serum_dummies <- names(pData(omics$data))[grep("Serum.run_order1_", names(pData(omics$data)))]
serum_dummies <- serum_dummies[-length(serum_dummies)]
pData(omics$data)[serum_dummies] <- lapply(pData(omics$data)[serum_dummies], as.factor)
...
denoise_vars_n <- c(serum_dummies, "e3_bw", "hs2_visit_age_years_Time1", "hs_dift_mealblood_imp",
                    "h_ethnicity_c_Caucasian", "h_ethnicity_c_Asian_pakistani","e3_sex", 
                    'cohort_BIB', "cohort_EDEN", "cohort_KANC")

denoise_vars_s <- c(serum_dummies, "e3_bw", "hs2_visit_age_years_Time1", 
                    "hs_dift_mealblood_imp", "e3_sex", "cohort_SAB")

denoise_vars <- c(serum_dummies, "e3_bw", "hs2_visit_age_years_Time1", "hs_dift_mealblood_imp",
                  "h_ethnicity_c_Caucasian", "h_ethnicity_c_Asian_pakistani","e3_sex", 'cohort_BIB',
                  "cohort_EDEN", "cohort_KANC", "cohort_MOBA", "cohort_SAB")
...
# ///// Save denoised data
#saveRDS(omics_n_denoised, file = paste0(output_path, "/metab_serum_denoised_n.RDS"))
#saveRDS(omics_s_denoised, file = paste0(output_path, "/metab_serum_denoised_s.RDS"))
saveRDS(omics_denoised, file = paste0(output_path, "/metab_serum_denoised.RDS"))
...
omics_n_denoised <- readRDS(paste0(output_path, "/metab_serum_denoised_n.RDS"))
omics_s_denoised <- readRDS(paste0(output_path, "/metab_serum_denoised_s.RDS"))
omics_denoised <- readRDS(paste0(output_path, "/metab_serum_denoised.RDS"))
